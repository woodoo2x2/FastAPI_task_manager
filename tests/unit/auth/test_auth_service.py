...
async def test_get_google_url__success(auth_service_mock: AuthService, settings: Settings):
    settings_google_redirect_url = settings.google_redirect_url

    auth_service_google_url = auth_service_mock.get_google_redirect_url()

    assert settings_google_redirect_url == auth_service_google_url


async def test_get_yandex_url__success(auth_service_mock: AuthService, settings: Settings):
    settings_yandex_redirect_url = settings.yandex_redirect_url

    auth_service_yandex_url = auth_service_mock.get_yandex_redirect_url()

    assert settings_yandex_redirect_url == auth_service_yandex_url


async def test_generate_jwt_token__success(auth_service_mock: AuthService, settings: Settings):
    user_id = 1
...
async def test_get_user_id_from_jwt_token__success(auth_service_mock: AuthService, settings: Settings):
    user_id = 1
    access_token = auth_service_mock.generate_access_token(user_id)
...
async def test_google_auth__success(auth_service_mock: AuthService, google_client, settings: Settings):
...
async def test_yandex_auth__success(auth_service_mock: AuthService, yandex_client, settings: Settings):
    user = await auth_service_mock.yandex_auth(code="fake_code")
